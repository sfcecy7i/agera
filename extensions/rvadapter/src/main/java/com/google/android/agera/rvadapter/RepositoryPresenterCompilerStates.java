...
public interface RepositoryPresenterCompilerStates {
  ...
  /**
   * Compiler state to optionally enable fine-grained events using {@link androidx.recyclerview.widget.DiffUtil}.
   */
  interface RPDiff<TVal, TRet> {

    /**
     * Specifies a key extractor {@link Function} to enable generating fine-grained events using
     * {@link androidx.recyclerview.widget.DiffUtil}. Two item objects are {@linkplain Callback#areItemsTheSame the same item} if
  ...
  /**
   * Compiler state to implementation for the specified item container type of the associated
   * {@link Repository}.
   */
  ...
  /**
   * Compiler state to implementation for the generic collection container type of the associated
   * {@link Repository}.
   */
